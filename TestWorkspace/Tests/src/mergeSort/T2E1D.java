...
public class T2E1D {
	...
	// post: array is sorted in ascending order (lowest to highest)
	{
		...
		if (array.length > 1) {
			...
			// e.g. if 7 elements total, sub-array 1 will have 3, and sub-array
			// 2 will have 4
			...
			// being compared
			// [k] stores the index of which element from arr2 is currently
			// being compared
			int i = 0, j = 0, k = 0;
			...
			// are no more
			// elements in it to compare. this means that all the elements in
			// the remaining
			// array are the highest (and sorted), so it's safe to copy them all
			// into the
			// final array.
			while (arr1.length != j) {
				array[i] = arr1[j];
