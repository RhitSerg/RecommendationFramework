	...
	private Collection<Edit> getSourceTargetEdits() {
		...
		Tree bestTargetMethodBody = Collections.max(targetMethodBodies, new Comparator<Tree>() {
			public int compare(Tree targetTree1, Tree targetTree2) {
				return (int) (PQGram.getDistance(sourceMethodBody, targetTree1.getChildren().get(6), Settings.P, Settings.Q) - PQGram.getDistance(sourceMethodBody, targetTree2.getChildren().get(6), Settings.P, Settings.Q));
			}
		});
