...
public class ProjectPickerDialog {
	...
	public ProjectPickerDialog(Shell shell) {
		this.shell = shell;
	}
	...
	private void createDialog() {
		...
		dialog.setText("Project Selection Dialog");
	...
	private void addTable() throws Exception {
		...
		IJavaModel javaModel = Activator.getJavaModel();
		IJavaProject[] javaProjects = javaModel.getJavaProjects();
		TreeSet<IJavaProject> set = new TreeSet<IJavaProject>(
				new Comparator<IJavaProject>() {
					public int compare(IJavaProject thisP, IJavaProject thatP) {
						return thisP.getElementName().compareTo(
								thatP.getElementName());
					}
				});
			...
			public void handleEvent(Event event) {
				if (event.widget == ok || event.widget == table) {
					...
					if (selection == null || selection.length == 0) {
						MessageDialog
								.openError(dialog, "Invalid Project Selection",
										"There was no selection of project. Please select again");
						return;
					}
					if (selection[0] == null) {
						MessageDialog
